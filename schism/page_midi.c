...
static void update_midi_values(void)
{
	...
	|       (widgets_midi[15].d.toggle.state ? MIDI_DETECT_CLOCK : 0)
...
static void get_midi_config(void)
{
	...
	widgets_midi[15].d.toggle.state = !!(midi_flags & MIDI_DETECT_CLOCK);
...
static void midi_page_redraw(void)
{
	...
	draw_text(        "Detect clock", 7, 36, 0, 2);
...
void midi_load_page(struct page *page)
{
	...
	create_toggle(widgets_midi + 6, 20, 35, 5, 15, 10, 10, 10, update_midi_values);
	...
	create_button(widgets_midi + 13, 2, 41, 27, 15, 14, 12, 12, 12,
	...
	create_toggle(widgets_midi + 15, 20, 36, 6, 13, 6, 6, 6, update_midi_values);
}
